      ...
      <div className={styles.basket__container}>
        ...
        <form className={styles.basket__form} action="">
          <p className={styles.basket__form__field}>
            <label className={styles.basket__form__label} htmlFor="name">
              Фамилия Имя Отчество
            </label>
            <input
              placeholder="Иванов Иван Иванович"
              className={styles.basket__form__input}
              type="text"
              id="name"
            />
          </p>
          <p className={styles.basket__form__field}>
            <label className={styles.basket__form__label} htmlFor="tel">
              Ваш телефон
            </label>
            <input
          ...
          <p className={styles.basket__form__field}>
            <label className={styles.basket__form__label} htmlFor="address">
              Адрес доставки{' '}
              <span>(Населенный пункт, улица, дом, корпус, квартира)</span>
            </label>
            <input
              placeholder="г. Минск, ул. Матусевича, д.30, кв. 1"
              className={styles.basket__form__input}
              type="text"
              id="address"
            />
          ...
          <p className={styles.basket__form__field}>
            <label className={styles.basket__form__label} htmlFor="">
              Способ доставки
            </label>
            {deliveryData.delivery?.map((item) => {
              return (
                <label
                  className={styles.basket__form__label__radio}
                  key={item.id}
                >
                  <input
                    className={styles.basket__form__input__radio}
                    type="radio"
                    name="delivery"
                  />
                  {item.title}
          ...
          <div className={styles.basket__price}>
            <p className={styles.basket__price__value}>
              Итоговая сумма:{' '}
              {basketData?.reduce(function (sum, elem) {
                return sum + elem.price;
              }, 0)}{' '}
              руб
            </p>
            <p className={styles.basket__info}>
